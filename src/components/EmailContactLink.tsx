...
export default function EmailContactLink({ email, className = '' }: EmailContactLinkProps) {
  ...
  return (
    <a
      ...
      className={`group relative py-0.5 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-colors duration-150 ease-[cubic-bezier(0.4,0,0.2,1)] break-all sm:break-normal ${className}`}
    >
      <span>{email}</span>
