...
export function Alert({
  ...
  return (
    <div
      role="alert"
      className={`relative w-full rounded-xl border p-3 sm:p-4 text-[12.5px] sm:text-[13.5px] transition-[border-color,background-color,color] duration-200 ease-[cubic-bezier(0.4,0,0.2,1)] shadow-xs flex items-start gap-2.5 sm:gap-3 ${variantStyles[variant]} ${className}`}
      {...props}
    >
      {children}
      {onClose && (
        <button
          ...
          className="shrink-0 p-1.5 sm:p-1 -mr-1 -mt-1 rounded-lg text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-200/50 dark:hover:bg-slate-800 active:scale-[0.96] transition-[transform,colors] duration-150 ease-[cubic-bezier(0.4,0,0.2,1)] cursor-pointer"
        >
